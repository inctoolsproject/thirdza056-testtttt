    ...
    try:
        ...
        if op.type == 26:
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["kickMention"] == True:
                     ...
                     balas = ["ฉันพูดไม่ได้ติดแท็กอีกครั้ง " + cName + "\nฉันเตะคุณ! ขอโทษนะ บ๊ายบาย!!!"]
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention"] == True:
                     contact = nadya.getContact(msg.from_)
                     cName = contact.displayName
                     balas = ["มีธระอะไรหรอครับ",cName + " ผมไม่ว่างครับ?",cName + " กำลังเล่นเกมอยู่","-_-","ฝากข้อความไว้นะ", cName + " ระบบตอบกลับอัตโนมัติ-ฝากข้อความไว้นะ" + cName, "ระบบตอบกลับอัตโนมัติ-ผมเล่นเกมอยุ่ " + cName, "ระบบตอบกลับอัตโนมัติ-ผมเล่นเกมอยุ่" + cName + "?", "ระบบตอบกลับอัตโนมัติ-ผมเล่นเกมอยุ่" + cName + "?","ระบบตอบกลับอัตโนมัติ-ผมเล่นเกมอยุ่ " + cName + " ระบบตอบกลับอัตโนมัติ-ผมเล่นเกมอยุ่!"]
                     ret_ = random.choice(balas)
                     name = re.findall(r'@(\w+)', msg.text)
                     mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                     mentionees = mention['MENTIONEES']
		...
		nadya.sendText(msg.to,"นั่นคือ Admin ของเรา (^_^)")	
              ...
              if msg.from_ in Creator:
                ...
                if targets == []:
                   nadya.sendText(msg.to,"ไม่พบติดต่อ")
                else:
                   for target in targets:
                        try:
                            admin.append(target)
                            nadya.sendText(msg.to,"Admin OK")
                        except:
                            pass
                print "[Command]เพิ่มผูดูแลระบบ"
              else:
                nadya.sendText(msg.to,"คำสั่ง ปฏิเสธ.")
                nadya.sendText(msg.to,"จำเป็นต้องได้รับอนุญาตจากผู้สร้าง.")
                
            elif "Admin remove @" in msg.text:
              if msg.from_ in Creator:
                print "[Command]ลบผูดูแลระบบ"
                ...
                if targets == []:
                   ...
                   for target in targets:
                        try:
                            ...
                            pass
                print "[Command]ผู้ดูแลระบบจะถูกลบออก"
              else:
                nadya.sendText(msg.to,"คำสั่ง ปฏิเสธ.")
                nadya.sendText(msg.to,"จำเป็นต้องได้รับอนุญาตจากผู้สร้าง.")
